...
class CRM_Mailing_Form_Unsubscribe extends CRM_Core_Form {
  ...
  public function preProcess() {
    ...
    if (!$job_id || !$queue_id || !$hash) {
      throw new CRM_Core_Exception('Brakuje parametrów');
    }

    // verify that the three numbers above match
    $q = CRM_Mailing_Event_BAO_MailingEventQueue::verify($job_id, $queue_id, $hash);
    if (!$q) {
      throw new CRM_Core_Exception('Wystąpił błąd podczas Twojej próby zweryfikowania prośby');
    }
    ...
    if (!$groupExist && !$isConfirm) {
      $statusMsg = sprintf('Adres e-mail %s został już wcześniej prawidłowy wypisany z newslettera.', [1 => $email]);
  ...
  public function buildQuickForm() {
    ...
    $this->setTitle('Potwierdź proszę wypisanie się z newslettera');
  ...
  public function postProcess() {
    ...
    $statusMsg = sprintf('Adres e-mail: %s został wypisany z newslettera.', [1 => $this->_email]);
